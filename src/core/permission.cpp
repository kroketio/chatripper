...
void Permission::setUID(const QByteArray &uid) {
    QWriteLocker locker(&mtx_lock);
    m_uid = uid;
    m_uid_str = Utils::uuidBytesToString(uid).toUtf8();
}

QByteArray Permission::uid() const {
    QReadLocker locker(&mtx_lock);
    return m_uid;
}

QByteArray Permission::role_uid() const {
    QReadLocker locker(&mtx_lock);
    return m_role_uid;
}

void Permission::setRoleUID(const QByteArray &role_uid) {
    QWriteLocker locker(&mtx_lock);
    m_role_uid = role_uid;
}
...
QSharedPointer<Permission> Permission::get_by_uid(const QByteArray &uid) {
...
QVariantMap Permission::to_variantmap() const {
    ...
    map["role_uid"] = QString::fromUtf8(m_role_uid);
...
rapidjson::Value Permission::to_rapidjson(rapidjson::Document::AllocatorType &allocator) const {
    ...
    obj.AddMember("role_uid", rapidjson::Value(m_role_uid.constData(), allocator), allocator);
